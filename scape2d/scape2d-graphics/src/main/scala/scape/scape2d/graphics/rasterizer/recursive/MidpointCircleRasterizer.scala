...
case class MidpointCircleRasterizer extends Rasterizer[Circle] {
  ...
  private def calculateZerothOctantOffsets(circle:Circle, y:Double, offsets:Set[Components2D]):Set[Components2D] = {
    val xSquared = (circle.radius * circle.radius) - (y * y);
    val x = sqrt(xSquared);
    if(x > y) calculateZerothOctantOffsets(circle, y + 1, offsets) + Components2D(x, y);
    else offsets;
  }
  
  private def reflectOctants(offset:Components2D, circle:Circle) = {
